...
if (!customElements.get('quick-order-list')) {
  customElements.define(
    ...
    class QuickOrderList extends HTMLElement {
      ...
      addMultipleDebounce() {
        this.querySelectorAll('quantity-input-quick').forEach((qty) => {
      ...
      addDebounce(id) {
        const element = this.querySelector(`#Variant-${id} quantity-input-quick`)
        const debouncedOnChange = debounce((event) => {
          this.onChange(event);
        }, ON_CHANGE_DEBOUNCE_TIMER);
...
if (!customElements.get('quantity-input-quick')) {
    customElements.define(
      'quantity-input-quick',
      class QuantityInputQuick extends HTMLElement {
    constructor() {
      super();
